...
> P.S(2025.11.12) : 위 결과를 자세히 보니 곽은지 담당자의 2024년 1월 데이터에 라우터가 누락이 되어 있습니다. 데이터를 읽어 분석하는 쪽에서의 누락이 강하게 의심이 되는 부분이었습니다. 
> 확인을 위해 gpt 시리즈(정확히는 gpt5-mini)와 Gemini(Gemini 2.5 Flash)를 번갈아 적용하면서 확인을 해 보았습니다. 확인 방법은 곽은지 담당자의 2024년 1월 매출 데이터를 찾아달라고 해서 결과를 비교했습니다 .
>
> gpt5-mini는 5회 실행시 2회는 데이터 누락이 발생했지만, Gemini2.5 Flash는 5회 모두 데이터 누락이 발생하지 않았습니다. Gemini는 오히려 데이터 정제를 자체적으로 해 주기까지 했습니다. 이는 gpt가 수치적 정확성을 요구하는 부분에서는 아직 미흡하다고 말을 할 수 있을 까요..? 어제 어떤 분이 gpt는 할루시네이션이 심해 perflexity로 넘어갔다고 하더니 저도 그래야 할 것 같습니다. 
